...
- **Interactive demos**: `demos/tetris.bf` is generated by `tools/generate_tetris_bf.php` as a gzip-compressed executable BF file with a `bfrun -@ -I -z` hashbang. Its tape is split into low control cells (`key`, command mailbox, active piece, next piece, delay, renderer scratch) and a fixed `10x20` board starting at cell `100`.
